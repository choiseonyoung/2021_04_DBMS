...
-- 데이터에 문제가 발생한 것이다. "참조 무결성이 무너졌다" 라고 한다.
...
-- Relation Ship 설정이 된 table간에 참조무결성을 보장하기 위한 제약조건을 설정할 수 있다
...
FOREIGN KEY(bk_ccode) -- FK를 설정할 컬럼, child의 컬럼
REFERENCES tbl_company(cp_code); -- 누구하고 parent table(컬럼)
...
-- 만약 tbl_books table의 bk_acode 컬럼에 저장된 데이터를 author에서 삭제하려고 하면
-- 오류가 발생하고 데이터가 삭제되지 않는다
DELETE FROM tbl_author
WHERE au_code = 'A0002';

-- A0001을 삭제한 상태에서 도서정보 추가
...
-- 데이터가 입력된 table간에 FK가 설정하려고 할 경우
