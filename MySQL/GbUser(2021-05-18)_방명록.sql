...
VALUE('2021-05-18','10:20:00','choiseonyoung','bviibb@naver.com','12345','내일은 석가탄신일');
...
-- 날짜와 시간 기준으로 최근글이 제일 먼저 보이도록 
SELECT * FROM tbl_guest_book
ORDER BY gb_date DESC, gb_time DESC;

-- UPDATE, DELECT를 수행할 때는 2개 이상의 레코드에 영향이 미치는 명령은 매우 신중하게 실행해야 한다
-- 가장 좋은 방법은 변경, 삭제하고자 하는 데이터가 여러개 있더라도 가급적 PK를 기준으로 1개씩 처리하는 것이다
UPDATE tbl_guest_book
SET gb_time = '10:36:00'
WHERE gb_seq = 2;
SELECT * FROM tbl_guest_book;
-- (UPDATE 기본키 있을 땐 기본키 대상으로 수행하는 게 가장 좋음)
