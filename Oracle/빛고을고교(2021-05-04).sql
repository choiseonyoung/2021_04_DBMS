...
SELECT ST.st_num AS 학번, ST.st_name AS 이름, ST.st_dpcode AS 학과코드, DP.dp_name AS 학과명, ST.st_grade AS 학년, ST.st_tel AS 연락처, ST.st_addr AS 주소
FROM tbl_student ST
...
(
    SELECT SC.sc_seq 일련번호, SC.sc_stnum 학번, ST.st_name 학생이름, ST.st_tel 전화번호, SC.sc_sbcode 과목코드, SB.sb_name 과목명, SC.sc_score 점수, SB.sb_prof 담임교수
    FROM tbl_score SC
...
SELECT 학번, 학생이름, 학과명, SUM(점수) AS 총점, ROUND(AVG(점수),1) AS 평균
FROM view_성적정보
GROUP BY 학번, 학생이름, 학과명
ORDER BY 학번;

-- DECODE() IF와 유사한 조건검색 함수
-- DECODE(컬럼명, 값, return)
--      컬럼명에 '값'이 담겨 있으면 return 명령을 수행하라
-- 과목명 컬럼에 국어 문자열이 담겨 있으면 해당 레코드의 점수 컬럼 값을 표시하고
--      그렇지 않으면 (null)을 표시하라

SELECT 학번,
    DECODE(과목명, '국어', 점수) AS 국어점수, -- 과목명에 들어있는 값이 국어면 점수 표시해라
    DECODE(과목명, '영어', 점수) AS 영어점수,
    DECODE(과목명, '수학', 점수) AS 수학점수
FROM view_성적정보
ORDER BY 학번;

-- 제2정규화 되어있는 Table의 데이터를 PIVOT하여 일반적인 보고서 List 형태로 만드는 코드
...
(
    SELECT 학번,
        SUM(DECODE(과목명, '국어', 점수)) AS 국어점수, -- 과목명에 들어있는 값이 국어면 점수 표시해라
