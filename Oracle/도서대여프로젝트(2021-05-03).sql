...
CREATE TABLE tbl_book_rent (
    ...
    br_sdate	nVARCHAR2(10)	NOT NULL,
    br_isbn	CHAR(13)	NOT NULL,
    br_bcode	CHAR(5)	NOT NULL,
    br_edate	nVARCHAR2(10),
    br_price	NUMBER		
);
...
ALTER TABLE tbl_book_rent -- N(다) table
ADD CONSTRAINT fk_isbn -- FK 이름
FOREIGN KEY(br_isbn) -- N table의 컬럼 tbl_book_rent에 들어있는 컬럼
...
-- 조건에 맞는(필요한) 데이터만 가져와서 그룹으로 묶고 ~

/*
위의 코드는 전체 데이터 중에서 대여일과 반납일에 조건을 부여한 후 데이터를 간추리고,
간추려진 데이터를 GROUP으로 묶어 보여주기

아래 코드는 전체 데이터를 GROUP으로 묶고
묶인 데이터를 조건에 맞는 항목만 보여주기

이 두 코드는 결과는 같지만 실행하는 성능은 매우 많은 차이가 난다
...
-- 전체 데이터 가져와서 그룹으로 묶고, 그룹으로 묶인 데이터 중에서 조건에 맞는 거 보여줌
...
SELECT 직업, SUM(급여)
FROM tbl_급여
WHERE BY 직업 != '영업직' AND 급여 > 1000000;
GROUP BY 직업;
...
VALUES ( seq_book_rent.NEXTVAL '2021-04-01','B0012','9791188850440');

INSERT INTO tbl_book_rent(br_seq, br_sdate, br_bcode, br_isbn)
VALUES ( seq_book_rent.NEXTVAL '2021-04-01','B0012','9791188850440');

-- 대여일이 2021-04-25일 이전이고 아직 미반납된 대여정보를 보여라
-- 그리고 회원코드순으로 보여라
SELECT 대여일, 회원코드, 회원명, 회원연락처, 도서명
FROM view_도서대여정보
WHERE 대여일 < '2021-04-25' AND 반납일 IS NULL
