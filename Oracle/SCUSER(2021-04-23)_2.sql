...
--  이 상태는 아직 Oracle Server의 저장소에 확정이 안 된 상태이다
--  확정이 안 된 상태에서 ROLLBACK 명령을 수행하면 모든 CUD(Insert, Update, Delete)가 취소된다
...
--  옵션 항목인 WHERE절을 반드시 포함하자
--  또한 특별하게 필요한 경우가 아니면
--  WHERE절에 포함하는 조건은 PK 컬럼을 기준으로 하자
...
-- VIEW를 만들 때 설정한 AS 값들이 컬럼이름으로 사용된다
SELECT * FROM view_score
WHERE 총점 < 200;
...
-- 먼저 영어점수 순으로 내림차순 정렬하고
-- 영어점수에 동점자가 있으면 수학점수 순으로 오름차순 정렬하라
-- 영어점수가, 수학점수 동점자가 있으면 국어점수 순으로 오름차순...
SELECT 학번, 
영어, 수학, 국어 FROM view_score
WHERE 총점 < 200
...
-- 학번은 단독으로 사용되었다
-- 이러한 데이터를 출력하려면
-- 함수로 감싸지 않은 컬럼들을 GROUP BY에 나열해 주어야 한다
SELECT 학번, MAX(총점)
FROM view_score
GROUP BY 학번;

-- GROUP BY
-- 같은 값을 갖는 데이터들끼리 묶어서 한 번만 출력하라
SELECT 총점
FROM view_score
GROUP BY 총점;
