...
-- 개수, 합산, 평균, 최소값, 최대값
-- 통계함수를 사용할 때 통계에 포함하지 않는 컬럼을 보고자 할 때는 GROUP BY를 사용하여 묶어줘야 한다
...
-- tbl_score 테이블을 나열하고
-- tbl_score의 sc_num 컬럼의 값과 같은 데이터를
-- tbl_stdent에서 찾아서 함께 나열하라
SELECT *
...
SELECT tbl_score.sc_num, tbl_student.st_name, tbl_student.st_dept, tbl_score.sc_kor, tbl_score.sc_eng, tbl_score.sc_math
FROM tbl_score, tbl_student
WHERE sc_num = st_num;

-- 테이블이름을 부착하기가 번거로우면
-- 테이블에 Alias를 추가한 후 각 칼럼 이름에 Alias를 사용할 수 있다
-- 테이블 Alias에도 AS 키워드를 사용하지만
-- 오라클에서는 테이블 Alias에 AS 키워드를 사용하면 문법오류가 발생한다
SELECT SC.sc_num, ST.st_name, ST.st_dept, SC.sc_kor, SC.sc_eng, SC.sc_math
...
-- 성적데이터는 모두 100개가 있지만
-- 학생데이터는 79개만 남아있기 때문에 JOIN한 결과가 학생 데이터와 같은 수인 79개만 조회(인출) 되고 있다
...
-- 학생데이터에는 1 ~ 79번까지만 있고
-- 성적데이터에는 1 ~ 100번까지 있다
-- 성적데이터의 80 ~ 100까지는 실제 존재하는 학생인지 아닌지 증명할 방법이 없다
-- 성적 데이터는 무결성이 깨진 상태가 된다
-- 학생테이블과 성적테이블 간의 연관(관계) 참조가 무너진 상태가 된다
...
-- 참조 무결성 여부와 관계없이 모든 데이터를 JOIN하여 보고 싶을 때

-- 참조 무결성에 문제가 있거나 하는 경우
-- LEFT JOIN(LEFT OUT JOIN)
--      tbl_score 테이블의 데이터는 모두 보여주고
--      학생 테이블에서 학번이 일치하는 학생이 있으면 같이 보여달라
-- 보통 Table의 참조 무결성 보증을 설정하는 경우가 있는데
-- 참조관계에 없는 다수의 테이블을 JOIN하여 보고 싶을 때는
-- LEFT JOIN을 사용한다.
...
-- 학생 데이터에 없는 학생의 성적이 추가되어 있는지 여부를 알아보고 싶을 때
...
-- 학생 데이터를 모두 나열하고
-- 학생 데이터와 일치하는 성적 데이터만 보여라
-- 학생 데이터와 성적 데이터 간의 참조 무결성에 오류가 있기 때문에
-- 실제 학생 데이터에 존재하는 성적 정보만 보고 싶을 때
SELECT COUNT(*)
FROM tbl_student
LEFT JOIN tbl_score
ON st_num = sc_num;
...
AS 
(
    SELECT SC.sc_num AS 학번, ST.st_name AS 이름, ST.st_dept AS 학과, ST.st_grade AS 학년, ST.st_tel AS 전화번호, SC_sc.kor AS 국어, SC.sc_eng AS 영어, SC.sc_math AS 수학,
           (SC.sc_kor + SC.sc_eng + Sc.sc_math) AS 총점, ROUND((SC.sc_kor + SC.sc_eng + Sc.sc_math)/3, 2) AS 평균
    FROM tbl_score SC
...
-- 학과순으로 정렬하고 같은 학과가 있으면 다시 총점순으로 정렬
SELECT * FROM view_성적정보
ORDER BY 학과, 총점 DESC;
