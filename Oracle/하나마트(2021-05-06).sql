...
SELECT
    SUM(DECODE(io_inout,'매입',io_total)) AS 매입합계,
    SUM(DECODE(io_inout,'매출',io_total)) AS 매출합계,
    SUM(DECODE(io_inout,'매출',io_total)) - SUM(DECODE(io_inout,'매입',io_total)) AS 이익금
FROM tbl_iolist;
...
-- 이 때 실제 쌀보다 더 많은 이익이 발생하게 되므로 가치가 부가 되었다 라고 표현한다
-- 가치가 부가된 만큼 세금을 납부하도록 한다
-- 부가 가치세(Value Add Tax, VAT)
-- 매입을 할 때는 매입금액의 10% 만큼으로 세금을 포함하여 매입하고
-- 매출을 할 때는 매출금액의 10% 만큼 세금을 포함하여 판매한다

-- 매입매출 데이터에서 보면
-- 매입금액은 부가세 10%을 제외한 금액으로 입력하고
-- 매출금액은 부가세 10%가 포함된 금액으로 입력한다
-- 샘플데이터의 매입금액은 VAT 제외금액이고, 매출금액은 VAT가 포함된 금액이다
...
SELECT SUM(DECODE(io_inout,'매입',ROUND(io_total * 0.1 ))) AS 매입부가세,
...
-- 거래처별로 매입과 매출 합계
-- DECODE(컬럼명, 조건값, true일 때, false일 때)
-- 실제 저장된 데이터를 PIVOT으로 보여주기
SELECT io_dname,
    SUM(DECODE(io_inout,'매입',io_total,0)) AS 매입합계, -- 있으면 '매입' 출력 없으면 0 출력해라
...
-- 2020-01-01부터 2020-06-30 기간동안
-- 거래된 리스트를 거래처별로 조회

-- 기간동안 거래처별 매입 매출 합계
SELECT io_dname, -- io_date을 넣으면 같은 거래처들이 여러번 나와버림. 날짜값은 WHERE에서 지정했음
...
-- 상품정보 테이블
-- DEFAULT 속성
-- INSERT를 수행할 때 값이 지정되지 않으면 자동으로 추가될 데이터
-- 자동으로 NOT NULL로 설정된다
...
그러나 현재 데이터는 코드 컬럼이 없이 이름 컬럼만 있는 상태이다

매입매출 데이터에 상품코드, 거래처코드 컬럼을 추가하고
