...
-- 학생테이블과 학생테이블을
-- 학생의 st_dcode 컬럼과 학과의 dp_code 컬럼을 연관지어
-- JOIN을 수행하라
--  학생테이블의 모든 데이터를 나열하고
...
-- 학과코드, 학과명으로 GROUP BY를 하면 학과별로 묶음이 이루어진다
-- 학과별로 묶음을 만들고 각 묶은 학과에 포함된 레코드가 몇개인가 세어보면,
-- 학과별 학생 인원수가 조회된다.
...
(
SELECT SC.sc_num 학번, ST.st_name 이름, ST.st_dcode 학과코드, DP.dp_name 학과명, DP.dp_prof 담당교수, ST.st_tel 전화번호,
