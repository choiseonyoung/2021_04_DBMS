...
-- io_inout 컬럼의 데이터가 1이면 매입을 표시
-- 아니면 매출을 표시하라
SELECT DECODE(io_inout,'1','매입','매출') AS 구분,
    COUNT(*)
FROM tbl_iolist
GROUP BY DECODE(io_inout,'1','매입','매출');
-- SELECT에 공식이 있으면 GROUP BY에서도 공식으로 묶어줘야 됨

-- 좀 더 정확하게하면 이렇게 할 수도 있음
-- IF io_inout == 1 THEN '매입'
-- ELSE IF io_inout == 2 THEN '매출'
-- IF io_inout == 1 THEN '매입'
-- ELSE '매출'
-- 오라클에선 if문이 일반 SELECT에서 못 쓰기 때문에 이렇게 씀
...
매입매출 데이터를 DB로 import한 후 테이블에서 상품정보와 거래처정보를 분리하여 제3정규화를 수행하기

현재 매입매출 테이블에 상품이름과 거래처명(대표포함)이 저장되어 있다
현재 테이블에서 만약 상품이름이나 거래처명이 변경되어야 하는 일이 발생한다면
다수의 데이터(레코드)에 변경(update)이 되는 상황이 만들어진다
...
매입단가가 0인 경우
매출단가에서 마진(20%)을 빼고, 다시 부가세(10%)를 뺀 가격으로 계산
매입단가 = (매출단가/1.2)/1.1
