...
-- 마치 excel의 sheet에 데이터를 저장하는 것처럼 데이터가 저장된다.
-- 실제 DBMS 소프트웨어가 저장하는 방식은 고유한 기술이 적용되어 있지만
-- DBMS 소프트웨어를 사용하는 사용자는 모든 데이터는
--      표(TABLE) 형식으로 저장된 것으로 인식하는 표현 형식이다
-- Oracle의 데이터 저장 구조 : TableSpace > Table 테이터들
-- TABLE은 Java 코딩에서 VO 클래스를 만드는 음악과 원리가 비슷하다
--      데이터를 취급하는 최소한의 규칙, 형식을 지정하여 생성한다
...
-- 데이터 데이터
-- 문자열 데이터
--      CHAR : 저장된느 길이가 (규칙으로) 정해진경우, 데이터 길이가 모두 같은 경우
--      VARCHAR : 저장하는 데이터의 최대크기만 지정할 수 있다
...
CREATE TABLE tbl_score (
    sc_num CHAR(5) , -- 문자열 5자리의 이내의 데이터만 저장, String num
