/*
iolist 테이블과 Prod 테이블간에 상품이름으로 JOIN을 하여 NULL값이 없는 것이 확인되었다

1. iolist 테이블에 상품코드 컬럼을 추가하고
...
-- 생성된 io_pcode 컬럼에 io_pname 컬럼의 상품이름에 해당하는 코드 데이터를 tbl_product에서 가져와서 저장을 해야한다.

-- 테이블의 데이터를 변경하기 위한 DML
-- tbl_iolist 전체를 반복하면서 io_pcode 컬럼에 값을 갱신하라
-- 이때 tbl_iolist의 상품으로 tbl_product 데이터를 조회하여 일치하는 데이터가 있으면
--      그 중에 상품코드 컬럼의 값을 가져와서 io_pcode 컬럼에 저장하라
UPDATE tbl_iolist IO
SET io_pcode = (
    SELECT p_code FROM tbl_product P
    WHERE IO.io_pname = P.p_name
);
-- 서브쿼리 이용해서 데이터 업데이트 (다중 서브쿼리)

/*
iolist 전체 데이터를 보여달라
iolist 데이터의 상품이름을 puoduct 테이블에서 조회하여 일치하는 상품이 있으면 리스트를 보일 때 같이 보여달라
라는 Sub Query
*/
SELECT IO.io_pname,
    (
...
iolist에 io_dcode 컬럼을 추가하고
데이터를 UPDATE 수행
*/

-- 1. iolist로부터 거래처명, 대표자명 컬럼을 기준으로 중복되지 않은 데이터를 조회
--      거래처명, 대표자명 순으로 정렬
-- projection : 기준이 되는 컬럼을 SELECT 표현
--      필요한 컬럼만 나타나며 전체 데이터가 출력
...
-- import된 거래처정보와 매입매출정보를 JOIN하여 NULL값이 있는지 확인
SELECT io_dname, d_name, d_ceo, d_code
FROM tbl_dept
    LEFT JOIN tbl_iolist
        ON io_dname = d_name AND io_dceo = d_ceo;

-- tbl_iolist에 io_dcode 컬럼추가, CHAR(5)
ALTER TABLE tbl_iolist
ADD io_dcode CHAR(5);

-- 거래처정보 테이블에서 거래처 코드를 조회하여
-- tbl_iolist의 io_dcode 컬럼에 UPDATE
UPDATE tbl_iolist
SET io_dcode = (
    SELECT d_code
    FROM tbl_dept
